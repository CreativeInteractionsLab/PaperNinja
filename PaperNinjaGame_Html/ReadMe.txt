...
PN uses Phaser as the game engine that does all the collision detections and whatnot. At the time of writing this document, Phaser version 2.6.2 is used (originally 2.4.3).
...
Html > phaser.2.6.2.min.js
Img > *.png
<Edit panel in the DroidScript IDE>
*.js
PaperNinja_<some_version_number>.html
...
The starting point of PN is the html file. The filename is not important, as long as it has the OnStart() function defined as a script component. What it does is to setup the app created by DroidScript so that it 1)loads all the javascript files, 2)connects to bluetooth for the bend sensors to work.

When PN starts, it first goes to OnStart(), does a bunch of setup, and loads all the Phaser stuff and the rest of the game code (*.js). During this process it will also associate all those game code to a "game" object provided by Phaser. 
...
To make it more consistent, all gestures are detected using the > threshold convention, and for up, this means flat_A[i] - A[i] > threshold; and for down this means A[i] - flat_A[i] > threshold. This require the bend sensors to be connected and placed in a uniformed way (+ve & -ve at the correct spot).